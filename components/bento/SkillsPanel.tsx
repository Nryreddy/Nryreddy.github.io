   ...
   return (
      <Panel className="p-4 md:p-6 bg-[#161619] shadow-2xl border-none col-span-1 md:col-span-8 overflow-hidden group">
         ...
         <div className="flex flex-col md:flex-row gap-4 md:gap-8 min-h-[500px]">
            ...
            <div className="w-full md:w-[280px] flex flex-col gap-2 shrink-0 relative z-20">
               {(Object.keys(skillData) as Array<keyof typeof skillData>).map((cat) => (
                  <button
                     ...
                     className={`relative w-full px-5 py-4 text-left rounded-xl transition-all duration-150 text-base font-black uppercase
                      ${activeCategory === cat
                  ...
                  >
                     {cat}
                     {activeCategory === cat && (
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-[#3FA684] shadow-[0_0_10px_#3FA684] animate-pulse"></span>
                     )}
                  </button>
               ))}
            </div>
